...
#### covid_rename: function that renames Covid-related variables. Covid-related 
# variables sometimes contain hyphens and sometimes 
# contains periods ("-' vs "."). In the hyphen case, they need to be renamed. 

covid_rename <- function(df = most_recent_df) {
  columns_to_rename <- list("obligated_amount_from_COVID-19_supplementals_for_overall_award" = "obligated_amount_from_COVID.19_supplementals_for_overall_award", 
  ...
  for (old_col in names(columns_to_rename)) {
    new_col <- columns_to_rename[[old_col]]
    if (old_col %in% colnames(df)) {
      df <- df %>% rename_with(~ new_col, all_of(old_col))
    }
  }
...
write_full_grants <- function(df = full_grants_df, URL = GRANTS_URL) {
  ...
  date_string <- substr(x = URL, start = 77, stop = 82)
  
  full_file_name <- paste0(USASPENDING_dir, 'full_grants_data_', date_string, 
                           '.feather')
...
export_usaspending_checks <- function(assert_full_grants_result,
  ...
  # Print a message to indicate success
  cat("Excel file '01-usa_spending_asserts.xlsx' has been created with 3 sheets.")
}
